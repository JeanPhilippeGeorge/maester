...
#>
function Get-MtExoThreatPolicyAntiPhish {
    [CmdletBinding()]
    [OutputType([object])]
    param()
    ...
    $Request = 'MtExoThreatPolicyAntiPhish'
    if ($null -eq $__MtSession.ExoCache.$Request) {
        Write-Verbose "$request not in cache, requesting."

        # Call directly 'Get-AntiPhishPolicy' to avoid changing the cache of 'Get-MtExo -Request AntiPhishPolicy'
        $policies = Get-AntiPhishPolicy
        $rules = Get-MtExo -Request AntiPhishRule
